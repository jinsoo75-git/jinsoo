...
| 9 | 단위/통합 테스트 작성 및 실행 | 전체 테스트 통과 (`pytest` 등) 확인 |
...
| 2 | 메인 화면 레이아웃 (카드 목록 + 추가 폼) 마크업 | 360px 너비에서 레이아웃 깨짐 없는지 확인 |
| 3 | Mac OS 톤 디자인 토큰 적용 (rounded-xl, shadow-lg, backdrop-blur 등) | `03-design.md` 토큰값과 실제 클래스 대조 |
| 4 | 라이트/다크 테마 토글 구현 (`localStorage`, `prefers-color-scheme`) | 새로고침 후 테마 유지 확인 |
| 5 | API 연결: 목록 조회 및 카드 렌더링 (status 배지, D-N HH:MM 표시) | 실제 백엔드 데이터가 화면에 정확히 표시되는지 확인 |
| 6 | API 연결: 추가/수정(모달)/삭제(휴지통+확인) 기능 구현 | CRUD 4종이 화면에서 각각 정상 동작하는지 수동 테스트 |
| 7 | 3초 폴링으로 목록 자동 갱신 구현 | 다른 클라이언트에서 변경 후 3초 내 반영 확인 |
| 8 | 전체 동작 확인 후 git commit & push | `git log`로 커밋 확인, 원격 저장소에 반영 확인 |
